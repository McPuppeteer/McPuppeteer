/**
 * Copyright (C) 2025 - PsychedelicPalimpsest Copyright (C) 2025 - Tweekeroo developers (Mostly
 * mesa)
 *
...
public class Freecam {
  ...
  public static final class FreecamMovementHandler {
    ...
    public void movementTick() {
      ...
      else if (!options.forwardKey.isPressed() && !options.backKey.isPressed()) sprinting = false;
    ...
    public static Vec3d calculatePlayerMotionWithDeceleration(
        Vec3d lastMotion, double rampAmount, double decelerationFactor) {
      GameOptions options = MinecraftClient.getInstance().options;
      int forward = 0;
      int vertical = 0;
      int strafe = 0;
      ...
      double verticalRamped =
          getRampedMotion(lastMotion.y, vertical, rampAmount, decelerationFactor);
      double strafeRamped =
    ...
    public static double getRampedMotion(
        double current, int input, double rampAmount, double decelerationFactor) {
      if (input != 0) {

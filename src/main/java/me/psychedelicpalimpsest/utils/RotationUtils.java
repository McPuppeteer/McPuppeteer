 ...
 * From:
 * https://github.com/cabaletta/baritone/blob/89b0fd74ddf8a9e3c2450fafd28267769e31d6a2/src/api/java/baritone/api/utils/RotationUtils.java
 *
...
public final class RotationUtils {
	...
	public static final Vec3d[] BLOCK_SIDE_MULTIPLIERS =
	    new Vec3d[] {new Vec3d(0.5, 0.0, 0.5), new Vec3d(0.5, 1.0, 0.5), new Vec3d(0.5, 0.5, 0.0),
			 new Vec3d(0.5, 0.5, 1.0), new Vec3d(0.0, 0.5, 0.5), new Vec3d(1.0, 0.5, 0.5)};
	...
	public static Optional<Vec3d> getCenterOfClosestFace(VoxelShape shape, Vec3d target) {
		...
		shape.forEachBox((minX, minY, minZ, maxX, maxY, maxZ) -> {
			// All 6 face centers
			Vec3d[] faceCenters = new Vec3d[] {// minX face
	...
	public static Rotation calcRotationFromCoords(BlockPos orig, BlockPos dest) {
		return calcRotationFromVec3d(new Vec3d(orig.getX(), orig.getY(), orig.getZ()),
					     new Vec3d(dest.getX(), dest.getY(), dest.getZ()));
	}

	public static Rotation wrapAnglesToRelative(Rotation current, Rotation target) {
		return current.yawIsReallyClose(target) ? new Rotation(current.getYaw(), target.getPitch())
	...
	public static Vec3d calculateBlockCenter(World world, BlockPos pos) {
		...
		if (shape.isEmpty()) {
			...
			if (!Double.isNaN(xDiff) && !Double.isNaN(yDiff) && !Double.isNaN(zDiff)) {
				if (b.getBlock() instanceof AbstractFireBlock) { yDiff = 0.0; }

				return new Vec3d((double) pos.getX() + xDiff, (double) pos.getY() + yDiff,
						 (double) pos.getZ() + zDiff);
			} else {

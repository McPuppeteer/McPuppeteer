...
public class MesaConfigUtils {
  ...
  */
  ...
  public static JsonObject handleGetMalilibConfigRequest(
      Map<String, List<? extends IConfigBase>> config, JsonObject request) {
    ...
    return BaseCommand.jsonOf(
        "value",
        getValueForMasaConfig(
            config, request.get("category").getAsString(), request.get("name").getAsString()));
  }

  public static JsonElement getValueForMasaConfig(
      Map<String, List<? extends IConfigBase>> config, String category, String name) {
  ...
  public static JsonObject handleSetMalilibConfigRequest(
      Map<String, List<? extends IConfigBase>> config, JsonObject request) {
  ...
  public static JsonObject setValueForMasaConfig(
      Map<String, List<? extends IConfigBase>> config,
      String catagory,
      String name,
      JsonElement value) {
    ...
    return BaseCommand.jsonOf(
        ...
        "message",
            "Cannot lookup value for Mesa style Malilib item '"
                + catagory
                + "'/'"
                + name
                + "'. Reason: "
  ...
  public static JsonObject handleExecMesaConfigRequest(
      Map<String, List<? extends IConfigBase>> config, JsonObject request) {
    if (!request.has("category") || !request.has("name")) {
      return BaseCommand.jsonOf(
          "status", "error",
    ...
    if (request.has("action") && request.get("action").isJsonPrimitive()) {
      ...
      else if (request.get("action").getAsString().equals("release")) action = KeyAction.RELEASE;
    }

    return executeHotkeyForMasaConfig(
        config, action, request.get("category").getAsString(), request.get("name").getAsString());
  }

  public static JsonObject executeHotkeyForMasaConfig(
      Map<String, List<? extends IConfigBase>> config,
      KeyAction action,
      String catagory,
      String name) {
      ...
      for (IConfigBase configItem : config.get(catagory)) {
        ...
        if (!(configItem instanceof IHotkey hotkey))
          return BaseCommand.jsonOf(
              "status", "error",
              "type", "unknown config item",
              "message", "'" + catagory + "'/'" + name + "' is not a hotkey!");
        ...
        if (!(hotkey.getKeybind() instanceof KeybindMulti))
          return BaseCommand.jsonOf(
              "status", "error",
              "type", "exception",
              "message", "An IKeybind that isn't KeybindMulti cannot be handled!");
        ...
        if (callback == null)
          return BaseCommand.jsonOf(
              "status", "error",
              "type", "exception",
              "message", "Callback is null, and thus cannot be handled!");

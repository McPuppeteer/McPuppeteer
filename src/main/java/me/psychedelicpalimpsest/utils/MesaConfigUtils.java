    ...
    */
	...
	public static JsonObject setValueForMasaConfig(Map<String, List<? extends IConfigBase>> config, String catagory, String name, JsonElement value) {
		...
		return BaseCommand.jsonOf(
		    ...
		    "message", "Cannot lookup value for Mesa style Malilib item '" + catagory + "'/'" + name + "'. Reason: " + (config.containsKey(catagory) ? "Category exists, item does not" : "Category does not exist"));
	}
	...
	public static JsonObject handleExecMesaConfigRequest(Map<String, List<? extends IConfigBase>> config, JsonObject request) {
		...
		if (request.has("action") && request.get("action").isJsonPrimitive()) {
			if (request.get("action").getAsString().equals("press"))
				action = KeyAction.PRESS;
			else if (request.get("action").getAsString().equals("release"))
				action = KeyAction.RELEASE;
		}

		return executeHotkeyForMasaConfig(config, action, request.get("category").getAsString(), request.get("name").getAsString());
	}

	public static JsonObject executeHotkeyForMasaConfig(Map<String, List<? extends IConfigBase>> config, KeyAction action, String catagory, String name) {
		if (config.containsKey(catagory))
			for (IConfigBase configItem : config.get(catagory)) {
				if (!configItem.getName().equals(name)) continue;
				if (!(configItem instanceof IHotkey hotkey)) return BaseCommand.jsonOf(
				    "status", "error",
				    "type", "unknown config item",

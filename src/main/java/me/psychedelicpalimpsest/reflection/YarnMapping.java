...
public class YarnMapping {
  ...
  private YarnMapping() {
    ...
    try (InputStream is =
        YarnMapping.class.getResourceAsStream("/assets/mc-puppeteer/mappings.tiny")) {
    ...
    if (!tree.getSrcNamespace().equals("official")) {
      throw new RuntimeException(
          "Invalid mappings format. Puppeteer requires the mappings format to have the official"
  ...
  public static <T extends Enum<T>> Optional<T> deserializeEnum(Class<T> cls, String str) {
    assert cls.isEnum();
    String mapped =
        YarnMapping.getInstance().mapFieldName(Namespace.NAMED, cls.getName(), str, null);
  ...
  public static <T extends Enum<T>> String serializeEnum(Enum<T> obj) {
    ...
    return YarnMapping.getInstance()
        .unmapFieldName(
  ...
  public MappingTree.ClassMapping unmapClass(String className) {
    String inter =
        resolver.unmapClassName("intermediary", className.replace('/', '.')).replace('.', '/');
   ...
   * @return the mapped field name, or {@code name} if no such mapping is present
   */
  public String unmapFieldName(
      Namespace targetNamespace, String owner, String name, String descriptor) {
    ...
    var field =
        clsF.getField(
            name, descriptor, fabricTree.getNamespaceId(resolver.getCurrentRuntimeNamespace()));
    if (field == null) return null;

...
public class McReflector {
  ...
  private static JsonElement serializeObject(Object obj, CircularRefHandler stack) {
    ...
    try {
      ...
      else if (obj instanceof Character) return typeWrap(obj, new JsonPrimitive((Character) obj));
            ...
            "recipe display",
                serializeObject(
                    entry
                        ...
                        .optionDisplay()
                        .getStacks(
                            new ContextParameterMap.Builder()
                                .build(new ContextType.Builder().build())),
                    stack));
      else if (obj instanceof ScreenHandlerType<?> type)
        return BaseCommand.jsonOf(
            "_TYPE", stringifyClassName(obj.getClass().getName()),
            "id", Registries.SCREEN_HANDLER.getId(type).toString());
      else if (obj instanceof NbtElement) return typeWrap(obj, new JsonPrimitive(obj.toString()));
      ...
      else if (obj.getClass().isEnum()) return new JsonPrimitive(serializeEnum((Enum<?>) obj));
      ...
      // This exists just to stop stack overflows
        ...
        return typeWrap(
            ...
            listToJsonArray(
                ((Collection<Object>) obj)
                    .stream().map((e) -> serializeObject(e, stack)).collect(Collectors.toList())));
        ...
        return typeWrap(
            obj, new JsonPrimitive(Base64.getEncoder().encodeToString(obj.toString().getBytes())));
  ...
  private static JsonObject typeWrap(Object obj, JsonElement element) {
    return BaseCommand.jsonOf(
        "_TYPE", stringifyClassName(obj.getClass().getName()), "data", element);
  }
  ...
  public static JsonObject serializeGenericObject(Object inputObj, CircularRefHandler stack) {
    ...
    object.addProperty(
        "_TYPE",
        YarnMapping.getInstance()
            .mapClassName(YarnMapping.Namespace.NAMED, inputObj.getClass().getName()));

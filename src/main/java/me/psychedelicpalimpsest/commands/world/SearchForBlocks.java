...
@PuppeteerCommand(
    cmd = "search for blocks",
    description = "Search the players render distance for a block by id",
    ...
    for (int i = 0; i < map.length(); i++) {
      ...
      for (var section : chunk.getSectionArray()) {
        ...
        for (int idx = 0; idx < palette.getSize(); idx++) {
          ...
          if (target.contains(Registries.BLOCK.getRawId(item.getBlock()))) results.put(idx, item);
        }
            ...
            .forEach(
                (id) -> {
                  ...
                  if (results.containsKey(id)) {
                    ...
                    var rel_loc =
                        computelocation(blockStateContainer.paletteProvider.edgeBits, secIdx[0]);
                    var global_loc =
                        new BlockPos(
                    ...
                    if (entity != null) {
                      obj.add(
                          "entity data",
                          McReflector.serializeObject(
                              entity.createNbtWithIdentifyingData(world.getRegistryManager())));
                    }

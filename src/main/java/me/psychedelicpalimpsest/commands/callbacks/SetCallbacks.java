  ...
  public void onRequest(JsonObject request, LaterCallback callback) {
    ...
    var typicalCallbacks =
        userCallbacks.entrySet().stream()
            .filter(entry -> CallbackManager.CALLBACK_STRING_TYPES.contains(entry.getKey()))
            .toList();
    if (packetCallbacks.stream()
        .anyMatch((entry) -> !CallbackManager.PACKET_LIST.contains(entry.getKey()))) {
    ...
    callback.callbacksModView(
        (callbackMap, packetMap) -> {

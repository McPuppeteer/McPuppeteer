...
@PuppeteerCommand(cmd = "get worlds",
		  description = "List ALL the worlds on this minecraft instances .minecraft folder."
...
public class GetWorlds implements BaseCommand {
	...
	public CompletableFuture<List<JsonObject>> getWorldListJson() {
		...
		return MinecraftClient.getInstance().getLevelStorage().loadSummaries(list).thenApply(
		    (levelSummaries -> {
			    ...
			    for (int i = 0; i < levelSummaries.size(); i++) {
					...
					"display name", levelSummary.getDisplayName(), "load name",
					levelSummary.getName(), "last played", levelSummary.getLastPlayed(), "version",
					levelSummary.getVersion().toString(), "details",
					levelSummary.getDetails().toString(), "icon path",
					levelSummary.getIconPath().toString(), "save path", save.getRootPath()));
			    }
			    return worlds;
		    }));
	}
	...
	public void onRequest(JsonObject request, LaterCallback callback) {
		new Thread(() -> {
			getWorldListJson().thenAccept(list -> {
				callback.resultCallback(BaseCommand.jsonOf("world count", list.size(), "worlds", list));
			});
		}).start();
	}
}

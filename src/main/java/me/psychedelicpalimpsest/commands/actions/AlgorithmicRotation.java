...
@PuppeteerCommand(
    ...
    description =
        "Smoothly rotates the player to the specified pitch and yaw using an optional interpolation"
            + " method and degrees per tick. Usage: {pitch: float, yaw: float, [interpolation:"
            + " string], [degrees per tick: float]}",
...
public class AlgorithmicRotation implements BaseCommand {
  ...
  static {
    ...
    map.put(
        "elasticOut",
        t -> (float) (Math.sin(-13 * Math.PI / 2 * (t + 1)) * Math.pow(2, -10 * t) + 1));
  ...
  public static void AlgorithmiclyRotate(
      float target_pitch,
      float target_yaw,
      float degrees_per_tick,
      String methodStr,
      RotOnError onError,
      RotOnSuccess onSuccess) {
    ...
    if (method == null) {
      onError.invoke(
          BaseCommand.jsonOf(
              "status", "error", "unexpected argument", "message", "Unknown interpolation method"));
      return;
    }
                ...
                if (Math.abs(end_pitch_diff) > 2f || Math.abs(end_yaw_diff) > 2f) {
                  onError.invoke(
                      BaseCommand.jsonOf(
                          ...
                          "type", "rotation error",
                          "message",
                              "An issue has occurred during algorithmic rotation. It is likely that"
                                  + " the user, or the server has caused a rotation that we didn't"
                                  + " expect! Pitch diff: "
                                  + end_pitch_diff
                                  + " Yaw diff: "
                                  + end_yaw_diff));

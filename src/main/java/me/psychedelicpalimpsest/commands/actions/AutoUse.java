...
public class AutoUse implements BaseCommand {
  ...
  @Nullable
  public static Vec3d handleAndGetPositionForRequest(
      int x, int y, int z, @Nullable String direction, UseOnError onError) {
    ...
    if (direction != null) {
      ...
      if (dirr == null) {
        onError.invoke(
            BaseCommand.jsonOf(
                "status", "error", "type", "expected argument", "message", "Invalid direction"));
        return null;
      }
  ...
  public static void AutomaticallyUse(
    ...
    PuppeteerTask.TaskEvent useEvent =
        (self, onCompletion) -> {
          if (p.squaredDistanceTo(point) > range * range) {
            onError.invoke(BaseCommand.jsonOf("status", "error", "type", "block range"));
            return;

...
@PuppeteerCommand(cmd = "set directional movement vector",
		  description = "Makes the player walk in a certain direction. This is equivalent 'set directional " +
				"movement degree' with atan(z, x); meaning that the vector will be normalized",
...
public class SetDirectionalInputVector implements BaseCommand {
	...
	public void onRequest(JsonObject request, LaterCallback callback) {
		MinecraftClient.getInstance().execute(() -> {
			setDirectionalInputVector(request.get("x").getAsFloat(), request.get("z").getAsFloat());

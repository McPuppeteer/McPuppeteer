...
@PuppeteerCommand(
    ...
    description =
        "Makes the player walk in a certain direction. This is equivalent 'set directional movement"
            + " degree' with atan(z, x); meaning that the vector will be normalized",
        ...
        .execute(
            () -> {
              setDirectionalInputVector(
                  request.get("x").getAsFloat(), request.get("z").getAsFloat());

...
@PuppeteerCommand(
    cmd = "set beacon effect",
    description = "Sets the effect of the beacon",
              ...
              MinecraftClient.getInstance()
                  ...
                  .sendPacket(
                      new UpdateBeaconC2SPacket(
                          request.has("primary")
                              ? parseEffect(request.get("primary").getAsString())
                              : Optional.empty(),
                          request.has("secondary")
                              ? parseEffect(request.get("secondary").getAsString())
                              : Optional.empty()));

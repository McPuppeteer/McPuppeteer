...
@PuppeteerCommand(
    cmd = "get player inventory",
    description = "Gets the players inventory",
...
public class GetPlayerInventory implements BaseCommand {
    ...
    if (!force
      ...
      callback.resultCallback(
          BaseCommand.jsonOf(
              ...
              "message",
                  "The player has a screen open that is not the inventory, any inventory features"
                      + " will not work correctly. This message can be disabled by setting"
                      + " force=true, but be warned not to use any slot commands."));
      return;
    }
  ...
  public static JsonObject getJson() {
    ...
    return BaseCommand.jsonOf(
        "slots", array, "name", inventory.getName().getString(), "type", "inventory");
  }
}
